...
class CookieCheckerGUI:
    ...
    def create_widgets(self):
        # Title
        title_label = ttk.Label(self.root, text="Bulk Cookie Checker", style='Header.TLabel')
        ...
        self.input_text = scrolledtext.ScrolledText(input_frame, height=8, width=80, font=('Courier', 9))
        ...
        ttk.Button(button_frame, text="Check Cookies", command=self.check_cookies_threaded).pack(side=tk.LEFT, padx=5)
        ...
        # Results tree
        columns = ('Status', 'Name', 'Value', 'Domain', 'Path', 'Expires', 'Secure', 'HttpOnly')
        ...
        self.tree.column('Value', anchor=tk.W, width=150)
        self.tree.column('Domain', anchor=tk.W, width=100)
        self.tree.column('Path', anchor=tk.W, width=80)
        self.tree.column('Expires', anchor=tk.W, width=100)
        self.tree.column('Secure', anchor=tk.W, width=70)
        self.tree.column('HttpOnly', anchor=tk.W, width=70)
    ...
    def load_from_file(self):
        ...
        if filepath:
            try:
                ...
                self.status_var.set(f"Loaded {len(content.splitlines())} cookies from file")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load file: {str(e)}")
    
    def check_cookies_threaded(self):
        """Run cookie checking in a separate thread"""
        thread = threading.Thread(target=self.check_cookies, daemon=True)
        thread.start()
    ...
    def check_cookies(self):
        ...
        self.status_var.set(f"Processing {len(cookie_strings)} cookies...")
        self.root.update()
        ...
        for idx, cookie_str in enumerate(cookie_strings):
            try:
                ...
                results.append(('Valid', parsed))
            except Exception as e:
                results.append(('Invalid', {'error': str(e), 'raw': cookie_str}))
            
            # Update status
            if (idx + 1) % 10 == 0:
                self.status_var.set(f"Processed {idx + 1}/{len(cookie_strings)}")
                self.root.update()
        ...
        self.status_var.set(f"Completed: {len(results)} cookies processed")
    ...
    def parse_cookie(self, cookie_str):
        ...
        return {
            ...
            'secure': '✓' if morsel['secure'] else '✗',
            'httponly': '✓' if morsel['httponly'] else '✗',
            'raw': cookie_str
        }
        ...
        if filepath:
            try:
                ...
                with open(filepath, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(['Status', 'Name', 'Value', 'Domain', 'Path', 'Expires', 'Secure', 'HttpOnly'])
